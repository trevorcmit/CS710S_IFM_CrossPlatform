...
{
    public class ViewModelTempGPSSetting : BaseViewModel
    {
        ...
        public ViewModelTempGPSSetting(IAdapter adapter, IUserDialogs userDialogs, IMvxNavigationService navigation) : base(adapter)
        {
            ...
            if (ind != null)
            {
                if ((int)ind == 1)
                {
                    switch (BleMvxApplication._rfMicro_TagType)
                    {
                        ...
                        case 1: // S3
                            //ShowViewModel<ViewModelTempGPSInventory>(new MvxBundle());
                            _navigation.Navigate<ViewModelTempGPSInventory>(new MvxBundle());
                        break;
                    }
                }
            }
